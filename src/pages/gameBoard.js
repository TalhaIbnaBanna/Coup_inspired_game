      ...
      <div class="my-cards">
        ${me.cards.map((c, i) => `
          <div class="my-card" style="--card-color: ${CHARACTER_INFO[c]?.color || '#666'}" data-card-index="${i}">
            <img src="${CHARACTER_INFO[c]?.image}" alt="${CHARACTER_INFO[c]?.name}" class="card-img" />
            <div class="my-card-name">${CHARACTER_INFO[c]?.name || c}</div>
          </div>
        `).join('')}
        ${me.deadCards.map(c => `
...
function renderResponseZone(state) {
  ...
  if (state.phase === 'block_window') {
    ...
    if (!hasChallenge && blockOptions.length === 0 && !hasPass) {
      zone.innerHTML = `<div class="waiting-message">Waiting for others to respond...</div>`;
      return;
    }
    ...
    zone.innerHTML = `
      <div class="response-panel">
        ...
        <p class="response-prompt">${actor?.name || 'Someone'} is acting. Respond?</p>
  ...
  if (state.phase === 'block_challenge') {
    ...
    if (!hasChallengeBlock && !hasPass) {
      zone.innerHTML = `<div class="waiting-message">Waiting for response...</div>`;
      return;
    }
    ...
    zone.innerHTML = `
      <div class="response-panel">
        <div class="response-timer" id="response-timer"></div>
        <p class="response-prompt">${blocker?.name || 'Someone'} is blocking. Challenge?</p>
        <div class="response-buttons">
