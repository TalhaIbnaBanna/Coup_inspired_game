...
const CARD_ABILITIES = {
  duke: ['👑 Tax (+3)', '🚫 Block Aid'],
  assassin: ['🗡️ Assassinate'],
  captain: ['💎 Steal (2)', '🛡️ Block Steal'],
  ambassador: ['🔄 Exchange', '🛡️ Block Steal'],
  contessa: ['🛡️ Block Kill']
};
...
export function renderGameBoard(container, data = {}) {
  container.innerHTML = `
    <div class="game-page">
      ...
      <!-- Event Log Sidebar & Overlay -->
      <div class="drawer-overlay event-overlay" id="event-log-overlay"></div>
      <div class="event-log-sidebar" id="event-log-sidebar">
        <div class="event-log-header">
          <h3 class="event-log-title">Event Log</h3>
          <button class="event-log-close" id="event-log-close" style="display: none;">✕</button>
        </div>
        <div class="event-log-list" id="event-log-list"></div>
      </div>

      <!-- Home FAB -->
      <button id="home-fab" class="fab fab-home" title="Home">🏠</button>
      ...
      <!-- Event Log FAB (Mobile only) -->
      <button id="event-fab" class="fab fab-event" title="Event Log">📋</button>

      <!-- Chat Drawer -->
      <button id="chat-fab" class="fab fab-chat" title="Chat">💬</button>
      <div id="chat-drawer" class="drawer drawer-right">
        <div class="drawer-overlay chat-overlay"></div>
        <div class="drawer-content chat-drawer-content">
  ...
  document.getElementById('home-fab').addEventListener('click', () => {
    if (confirm('Leave the game and return to home screen?')) {
      window.location.reload();
    }
  });

  // Event log mobile drawer
  const eventFab = document.getElementById('event-fab');
  const eventSidebar = document.getElementById('event-log-sidebar');
  const eventOverlay = document.getElementById('event-log-overlay');
  
  const toggleEventLog = () => {
    eventSidebar.classList.toggle('mobile-open');
    if (eventOverlay) {
      eventOverlay.style.opacity = eventSidebar.classList.contains('mobile-open') ? '1' : '0';
      eventOverlay.style.pointerEvents = eventSidebar.classList.contains('mobile-open') ? 'all' : 'none';
      eventOverlay.style.zIndex = '940';
    }
  };

  eventFab?.addEventListener('click', toggleEventLog);
  document.getElementById('event-log-close')?.addEventListener('click', toggleEventLog);
  eventOverlay?.addEventListener('click', toggleEventLog);
  ...
  document.querySelector('.chat-overlay')?.addEventListener('click', () => {
  ...
  const chatHandler = (data) => {
    const isMe = data.playerId === socketClient.id;
    const msgEl = document.createElement('div');
    msgEl.className = `chat-msg ${isMe ? 'msg-me' : 'msg-them'}`;
    msgEl.innerHTML = `
      <div class="msg-sender">${data.playerName}</div>
      <div class="msg-text">${data.message}</div>
    `;
    chatMessages.appendChild(msgEl);
    chatMessages.scrollTop = chatMessages.scrollHeight;
  };
  socketClient.off('chatMessage', chatHandler);
  socketClient.on('chatMessage', chatHandler);
...
function updateGameBoard(state) {
  ...
  renderAnnouncementBanner(state);
        ...
        ${me.cards.map((c, i) => `
          <div class="my-card-container">
            ...
            <div class="card-abilities">
              ${(CARD_ABILITIES[c] || []).map(a => `<span class="ability-pill" style="--pill-color: ${CHARACTER_INFO[c]?.color || '#666'}">${a}</span>`).join('')}
            </div>
          </div>
        `).join('')}
        ${me.deadCards.map(c => `
...
function renderResponseZone(state) {
  ...
  if (state.phase === 'block_challenge') {
    ...
    const blockChar = state.turnData?.blockCharacter;
...
function renderAnnouncementBanner(state) {
  const banner = document.getElementById('announcement-banner');
  if (!banner) return;

  if (!state.lastEvent || state.phase === 'game_over') {
    banner.innerHTML = '';
    banner.classList.remove('visible');
    return;
  }

  // Only animate when it's a new event
  if (state.lastEvent.timestamp !== lastBannerTimestamp) {
    lastBannerTimestamp = state.lastEvent.timestamp;
    banner.innerHTML = `<div class="banner-text">${state.lastEvent.message}</div>`;
    banner.classList.remove('visible');
